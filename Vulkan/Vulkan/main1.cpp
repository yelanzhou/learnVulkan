...
int main()
{
    ...
    auto surfacePtr = std::make_shared<VulkanSurface>(window, VulkanGlobal::g_vulkanInstancePtr);

    auto devicePtr = std::make_shared<VulkanDevice>(VulkanGlobal::g_vulkanInstancePtr, VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu());
    devicePtr->initDevice();
    auto swapChainPtr = std::make_shared<VulkanSwapChainKHR>(VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu(), surfacePtr, devicePtr, width, height);
    ...
    //vertex
    std::shared_ptr<VulkanHostVisibleVertexBuffer> vertexBufferPtr = std::make_shared<VulkanHostVisibleVertexBuffer>(devicePtr, VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu(), vertices);
    uint64_t bufferSize = static_cast<uint64_t>(sizeof(VertexData)) * vertices.size();
    std::shared_ptr<VulkanDeviceLocalVertexBuffer> localDeviceVertexPtr = std::make_shared<VulkanDeviceLocalVertexBuffer>(devicePtr, VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu(), bufferSize);
    VulkanBuffer::Copy(vertexBufferPtr, localDeviceVertexPtr, bufferSize);


    //index 
    auto indexBufferPtr = std::make_shared<VulkanHostVisibleIndexBuffer>(devicePtr, VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu(), indices);
    bufferSize = static_cast<uint16_t>(sizeof(uint16_t) * indices.size());
    auto deviceLocalIndexBufferPtr = std::make_shared<VulkanDeviceLocalIndexBuffer>(devicePtr, VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu(), bufferSize);
    ...
    auto uniformBufferPtrs = std::vector<std::shared_ptr<VulkanUniformBuffer>>(swapChainPtr->getImageViews().size(), std::make_shared<VulkanUniformBuffer>(devicePtr, VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu(), data));
    ...
    auto imageBufferPtr = std::make_shared<VulkanImageHostVisibleBuffer>(devicePtr, VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu(), imageData);
    auto imagePtr = std::make_shared<VulkanDeviceLocalImage>(devicePtr, VulkanGlobal::g_vulkanGpuManagerPtr->getSuitableGpu(), imageData);
